 ...
 *  	- checked exception(non RuntimeException) : 발생가능한 코드 블럭내에서 어떤 형태로든 해당 예외를 처리해야 하고,
 *  												처리하지 않으면 컴파일 에러가 발생함
 *  	- unChecked exception : RuntimeException 의 하위 예외들로 주로 JVM이 발생시키고,
 ...
 *  - 직접 처리 : try(Closable 객체 생성)~catch(exception..)~finally
 *  try{
 *  	// 잠재적 예외 발생 코드
 *  }catch(처리할 예외들 선언){
 *  	// 직접 처리
 ...
 * 			}
 *  	- 예외 복구 : 예외발생시 일정 시간 지연 후 해당 작업 재실행
 *  }finally{
